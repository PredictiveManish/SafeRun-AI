...
"""
    ...
    print(
        f"Dangerous import: risk_level={result.risk_level}, warnings={result.warnings}"
    )
    assert result.risk_level == "BLOCKED", (
        f"Expected BLOCKED risk, got {result.risk_level}"
    )
    assert any("Dangerous import: os" in w for w in result.warnings), (
        f"Expected os import warning, got {result.warnings}"
    )
...
"""
    result = scanner.scan(safe_code)
    print(
        f"Safe import from: risk_level={result.risk_level}, warnings={result.warnings}"
    )
    assert result.risk_level == "LOW", f"Expected LOW risk, got {result.risk_level}"
...
"""
    result = scanner.scan(dangerous_code)
    print(
        f"Dangerous import from: risk_level={result.risk_level}, warnings={result.warnings}"
    )
    assert result.risk_level == "BLOCKED", (
        f"Expected BLOCKED risk, got {result.risk_level}"
    )
    assert any("Dangerous import from: os" in w for w in result.warnings), (
        f"Expected os import from warning, got {result.warnings}"
    )
...
"""
    ...
    print(
        f"Suspicious string: risk_level={result.risk_level}, warnings={result.warnings}"
    )
    # Should have at least LOW risk due to the suspicious path
    assert result.risk_level in ["LOW", "MEDIUM", "HIGH", "BLOCKED"], (
        f"Expected some risk, got {result.risk_level}"
    )
    assert any("Suspicious path reference" in w for w in result.warnings), (
        f"Expected suspicious path warning, got {result.warnings}"
    )
...
"""
    ...
    assert any("Potential infinite loop" in w for w in result.warnings), (
        f"Expected infinite loop warning, got {result.warnings}"
    )
...
"""
    ...
    print(
        f"Loop with break: risk_level={result.risk_level}, warnings={result.warnings}"
    )
    # May still have other warnings, but should not have infinite loop warning
    infinite_loop_warnings = [
        w for w in result.warnings if "Potential infinite loop" in w
    ]
    assert len(infinite_loop_warnings) == 0, (
        f"Expected no infinite loop warnings, got {infinite_loop_warnings}"
    )
